...
function removeFirstOccurrences(str, value) {
  const fstI = str.indexOf(value);
  if (fstI !== -1) {
    return (
      str.substring(0, fstI) + str.substring(fstI + value.length, str.length)
    );
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const lastI = str.lastIndexOf(value);
  if (lastI !== -1) {
    return str.substring(0, lastI) + str.substring(lastI + value.length);
  }
  return str;
}
...
function orderAlphabetically(str) {
  const alfStr = str.split('');
  // for (let i = 0; i < alfStr.length - 1; i += 1) {
  //   if (alfStr[i] > alfStr[i + 1]) {
  //     [alfStr[i], alfStr[i + 1]] = swap(alfStr[i], alfStr[i + 1]);
  //   }
  // }
  return alfStr.sort().join('');
}
/**
...
function isPalindrome(str) {
  ...
  return (
    strL.substring(0, Math.ceil(str.length / 2)) ===
    reverseString(strL.substring(Math.floor(str.length / 2), str.length))
  );
...
function reverseWords(str) {
  ...
  let newStr = '';
  for (let i = 0; i < splitStr.length; i += 1) {
    newStr += reverseString(splitStr[i]);
...
function invertCase(str) {
  ...
  for (let i = 0; i < splitStr.length; i += 1) {
    ...
    } else if (splitStr[i] >= 'a' && splitStr[i] <= 'z') {
